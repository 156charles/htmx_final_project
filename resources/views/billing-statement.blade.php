                    ...
                    @foreach ($char as $charge)
                        <div class="flex relative w-full justify-between">
                            ...
                            @php
                                $total += $charge->amount;
                            @endphp
                            <form class="absolute -right-[5rem]" hx-delete="/api/charge/{{$charge->id}}"  hx-confirm="Are you sure you want to delete this charge?">
                                @csrf
                                @method('DELETE')
    ...
    <div class="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50 hidden"
        ...
        <div class="bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:max-w-lg sm:w-full">
            <div class="px-4 py-5 sm:p-6">
                <h3 class="text-xl font-bold text-gray-900">Create Charge for Account {{ $account->student->first_name }} - {{ $account->student->last_name }}</h3>
